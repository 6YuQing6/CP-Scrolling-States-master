...
class IdleState extends State {
    ...
    execute(scene, hero) {
        ...
        if (Phaser.Input.Keyboard.JustDown(FKey)) {
            ...
            return // kicks out of existing class, so wont cascade further down
        }
    }
}
...
class CircularState extends State {
    enter(scene, hero){ // odes something when enters state
        //hero.setTint(0x0000FF)
        hero.setVelocity(0)
        hero.anims.play('circular-attack').once('animationcomplete', () => { // once animation ends transition back to idle
